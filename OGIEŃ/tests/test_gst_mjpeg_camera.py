...
class TestGstMjpegEnv(unittest.TestCase):
    ...
    def test_hw_jpeg_off_in_docker(self) -> None:
        with mock.patch('release.gst_mjpeg_camera._in_docker', return_value=True):
            with mock.patch.dict(os.environ, {'DRONIADA_GST_HW_JPEG': '1'}, clear=False):
                self.assertFalse(use_hw_jpeg())

    def test_gst_capture_off_by_default_in_docker(self) -> None:
        with mock.patch('release.gst_mjpeg_camera._gst_available', return_value=True):
            with mock.patch('release.gst_mjpeg_camera._in_docker', return_value=True):
                with mock.patch.dict(os.environ, {}, clear=True):
                    os.environ['DRONIADA_CAMERA_FOURCC'] = 'MJPG'
                    self.assertFalse(should_use_gst_capture())
                with mock.patch.dict(os.environ, {'DRONIADA_USE_GST_CAPTURE': '1'}, clear=False):
                    self.assertTrue(should_use_gst_capture())
