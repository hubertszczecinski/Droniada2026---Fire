...
def use_hw_jpeg() -> bool:
    ...
    W Dockerze headless nvvidconv wymaga EGL (``EGL failed to initialize``) —
    domyślnie software ``jpegdec``/``jpegenc``.
    """
...
def should_use_gst_capture() -> bool:
    ...
    raw = os.environ.get('DRONIADA_USE_GST_CAPTURE', '').strip().lower()
    if raw in ('0', 'false', 'no', 'off'):
        return False
    if not raw and _in_docker():
        return False
    if not raw:
        raw = '1'
