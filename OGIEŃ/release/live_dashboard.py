"""Dashboard live: moduł A + B, galeria migawek."""
...
_DASHBOARD_WINDOW_SIZE: Optional[Tuple[int, int]] = None
_PARAMS_COL_A_LINES = 9
_PARAMS_COL_B_FIELDS = 10
_PARAMS_COL_C_SNAP_LINES = 4
_DASHBOARD_TOP_H_MIN = 720
_PARAMS_H_MIN = 500
_PARAMS_TITLE_H = 58
_STRIP_H = 130
_GAP = 10
...
_FONT_HEADER = 1.12
_FONT_TITLE = 1.32
_REPROJ_DISPLAY_MAX_PX = 60.0
_PANEL_PRESENCE_REASON_PL = {
    'ok': '',
    'reliable_b': 'moduł B reliable',
    'ok_reproj_b': 'niski reproj modułu B',
    'no_corners': 'brak rogów YOLO',
    'tracker_hold': 'tracker trzyma stare rogi',
    'high_reproj': 'zbyt wysoki reproj rogów',
    'low_yolo_conf': 'niska pewność YOLO',
...
def pose_reproj_display_str(reproj_a: float, reproj_b: float) -> str:
    ra = float(reproj_a)
    if np.isfinite(ra) and ra < _REPROJ_DISPLAY_MAX_PX:
        return f'{ra:.1f} px'
    rb = float(reproj_b)
    if np.isfinite(rb) and rb < _REPROJ_DISPLAY_MAX_PX:
        return f'{rb:.1f} px'
    return 'n/d'
...
def _params_cols_bottom(y0: int, lay: _ParamsLayout, lh: int) -> int:
    body = y0 + lay['sec_step']
    col_h = body + max(_PARAMS_COL_A_LINES, _PARAMS_COL_B_FIELDS, _PARAMS_COL_C_SNAP_LINES) * lh
    return col_h + 10
...
def _field_value_x(x: int, labels: List[str], scale: float, *, gap: int = 12) -> int:
    if not labels:
        return x + gap
    w = max(_text_width(f'{lb}:', scale, 2) for lb in labels)
    return x + w + gap
...
def _put_field(
    ...
    *,
    ...
    ls = label_scale if label_scale is not None else _FONT_MUTED
    vs = value_scale if value_scale is not None else _FONT_MAIN
    put_text_utf8(canvas, f'{label}:', (x, y), _MUTED, scale=ls, thickness=2)
    put_text_utf8(canvas, value, (value_x, y), color, scale=vs, thickness=2)
    return y + lh
...
def _draw_cxy_list(
    ...
    *,
    ...
    if len(preds) > limit:
        y = _put_line(
            canvas, y, f'{prefix}+{len(preds) - limit} wiecej', color=_MUTED, scale=font_muted, x=x,
            line_h=line_h,
        )
    return y
...
def _draw_bottom_params_panel(
    ...
    *,
    ...
    cols_bottom = _params_cols_bottom(y0, lay, lh)
    panel_h = max(lay['min_h'], cols_bottom + pad)
    panel = np.full((panel_h, width, 3), _SIDE_BG, dtype=np.uint8)
    cv2.rectangle(panel, (0, 0), (width, title_h), (210, 210, 210), -1)
    put_text_utf8(
        panel, 'PODGLĄD LIVE', (pad, int(title_h * 0.72)), _TEXT, scale=ft, thickness=3,
    )
    ...
    y_a = y0
    val_x_a = _field_value_x(
        x_a,
        ['W kadrze', 'Status', 'Odległość', 'Stojak', 'Pewność', 'Roll', 'Pitch', 'Yaw', 'Reproj A'],
        fmu,
    )
    y_a = _section_header(
        panel, y_a, 'MODUŁ A:', _SECTION_A,
    ...
    if panel_present:
        ...
        reason_pl = _PANEL_PRESENCE_REASON_PL.get(
            panel_presence_reason, panel_presence_reason,
        )
        w_kadrze_val = f'NIE ({reason_pl})' if reason_pl else 'NIE'
        w_kadrze_color = _FAIL
    y_a = _put_field(
        panel, y_a, 'W kadrze', w_kadrze_val,
        x=x_a, value_x=val_x_a, color=w_kadrze_color,
        label_scale=fmu, value_scale=fm, line_h=lh,
    )
    if module_a is None:
        ...
        y_a = _put_field(
            panel, y_a, 'Status', 'OK', x=x_a, value_x=val_x_a, color=_OK,
            label_scale=fmu, value_scale=fm, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Odległość', f'{module_a.distance_m:.2f} m',
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fm, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Stojak', f'{pl} ({module_a.report_angle_deg}°)',
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fm, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Pewność', f'{module_a.stand_confidence:.0%}',
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fmu, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Roll', f'{module_a.roll_deg:.0f}°',
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fmu, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Pitch', f'{module_a.pitch_deg:.0f}°',
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fmu, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Yaw', f'{module_a.yaw_deg:.0f}°',
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fmu, line_h=lh,
        )
        y_a = _put_field(
            panel, y_a, 'Reproj A',
            pose_reproj_display_str(module_a.reproj_mean_px, reproj_b),
            x=x_a, value_x=val_x_a, label_scale=fmu, value_scale=fmu, line_h=lh,
        )
        module_lines = 8
    else:
        ...
        y_a = _put_field(
            panel, y_a, 'Status', 'FAIL', x=x_a, value_x=val_x_a, color=_FAIL,
            label_scale=fmu, value_scale=fm, line_h=lh,
        )
        y_a = _put_line(panel, y_a, reason, color=_MUTED, scale=fmu, x=x_a, line_h=lh)
        module_lines = 2
    y_a += max(0, 8 - module_lines) * lh

    x_b = col_bounds[1][0] + pad
    b_labels = [
        'Reliable', 'Reproj B', 'Inliers', '3 siatki', 'Linie',
        'ROI rogów', 'Warp', 'Rogi', 'Backend', 'Panel',
    ]
    val_x_b = _field_value_x(x_b, b_labels, fmu)
    y_b = y0
    y_b = _section_header(
        panel, y_b, 'MODUŁ B:', _SECTION_B,
        x0=col_bounds[1][0], x1=col_bounds[1][1], layout=lay,
    )
    y_b = _put_field(
        panel, y_b, 'Reliable', 'TAK' if reliable else 'NIE',
        x=x_b, value_x=val_x_b, color=_OK if reliable else _FAIL,
        label_scale=fmu, value_scale=fm, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Reproj B', f'{reproj_b:.1f} px',
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fm, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Inliers', str(homography_inliers),
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fm, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, '3 siatki', _pct_label(grid_overlap_ratio),
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fmu, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Linie', _pct_label(grid_line_match_ratio),
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fmu, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'ROI rogów', _pct_label(roi_coverage),
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fmu, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Warp', _pct_label(warp_panel_coverage),
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fmu, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Rogi', corner_source,
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fmu, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Backend', xy_backend,
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fmu, line_h=lh,
    )
    y_b = _put_field(
        panel, y_b, 'Panel', panel_id,
        x=x_b, value_x=val_x_b, label_scale=fmu, value_scale=fm, line_h=lh,
    )
    ...
    y_c = y0
    y_c = _section_header(
        panel, y_c, 'MIGAWKI', _SECTION_SNAP,
        x0=col_bounds[2][0], x1=col_bounds[2][1], layout=lay,
    )
    for slot in range(_PARAMS_COL_C_SNAP_LINES):
        if slot < len(snapshot_entries):
            ent = snapshot_entries[slot]
            extra = ''
        ...
        elif slot == 0 and not snapshot_entries:
            y_c = _put_line(
                panel, y_c, 'Brak (reliable + reproj)', color=_MUTED, scale=fmu, x=x_c, line_h=lh,
            )
        else:
            y_c += lh
...
def _draw_snapshot_strip(
    ...
    if not entries:
        put_text_utf8(
            strip,
            'Galeria migawek: reproj B <= próg i reliable=TAK',
            (12, _STRIP_H // 2),
            (190, 190, 190),
            scale=0.52,
            thickness=2,
...
def compose_unified_dashboard(
    ...
    *,
    ...
    panel_title = 'Moduł B: podgląd'
    if not live_preds:
        panel_title += ' (bez kartek live)'
    ...
    main_h = max(_DASHBOARD_TOP_H_MIN, camera_vis.shape[0], panel_vis.shape[0])
...
def snapshot_frame_eligible(
    *,
    ...
    elif block_unreliable_zero_inl and int(homography_inliers) <= 0:
        overlap_need = max(overlap_need, float(min_grid_overlap_if_unreliable))
...
def snapshot_scene_eligible(
    *,
    panel_present: bool,
    corner_source: str,
    require_yolo_corners: bool = False,
    module_a_stand_confidence: Optional[float] = None,
    min_stand_confidence: float = 0.0,
) -> Tuple[bool, str]:
    """Scena (nie metryki siatki): panel w kadrze, nie cv_fallback na trawie."""
    if not panel_present:
        return False, 'no_panel'
    src = str(corner_source or '').lower()
    if require_yolo_corners:
        if 'cv_fallback' in src and 'yolo_pose' not in src:
            return False, 'cv_fallback'
        if 'yolo' not in src:
            return False, 'no_yolo_corners'
    if min_stand_confidence > 0.0 and module_a_stand_confidence is not None:
        if float(module_a_stand_confidence) < float(min_stand_confidence):
            return False, 'low_stand_conf'
    return True, 'ok'
...
def show_dashboard_window(img: np.ndarray, target_w: int = 1600) -> None:
    ...
    win_h = min(show.shape[0], 960)
    win_size = (show.shape[1], win_h)
    if _DASHBOARD_WINDOW_SIZE != win_size:
        _DASHBOARD_WINDOW_SIZE = win_size
        cv2.resizeWindow(_DASHBOARD_WINDOW, win_size[0], win_size[1])
