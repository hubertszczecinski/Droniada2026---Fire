...
from release.live_dashboard import pose_reproj_display_str
...
<p class="hint">Stream: kamera + siatka YOLO (~30&nbsp;fps). Parametry i raporty: <a href="/live.jpg" target="_blank">/live.jpg</a> · sterowanie <strong>:8089</strong>.</p>
<script>
let state = {};
let _mjpegOk = true;
...
<main>
  ...
  <div class="card" id="snapshotPanel">
    ...
    <div class="tune-row">
      <label>Min. zgodność 3 siatek <span id="vSnapOverlap">45</span>%</label>
      <input id="snapOverlap" type="range" min="30" max="98" step="1" value="45"/>
    </div>
    <p class="tune-hint">Im wyżej, tym trudniej o migawkę. Domyślnie 45% (zawody / lot).</p>
    <div class="tune-row">
      <label>Min. 3 siatek (unreliable) <span id="vSnapOverlapUnrel">45</span>%</label>
      <input id="snapOverlapUnrel" type="range" min="30" max="98" step="1" value="45"/>
    </div>
    <p class="tune-hint">Gdy reliable=NIE i brak inlierów: osobny, ostrzejszy próg (domyślnie = zgodność).</p>
    <div class="tune-row">
      <label>Klatek z rzędu <span id="vSnapStable">1</span></label>
      <input id="snapStable" type="range" min="1" max="5" step="1" value="1"/>
    </div>
    <p class="tune-hint">Ile kolejnych klatek musi spełniać próg, zanim zapisze migawkę (filtr drgań).</p>
    <div class="tune-row">
      <label>Max reproj B <span id="vSnapReproj">25</span> px</label>
      <input id="snapReproj" type="range" min="5" max="40" step="1" value="25"/>
    </div>
    <p class="tune-hint">Maks. błąd reprojekcji rogów po warp (px).</p>
    <div class="tune-row">
      <label>Min. pokrycie warpu <span id="vSnapWarp">15</span>%</label>
      <input id="snapWarp" type="range" min="0" max="40" step="1" value="15"/>
    </div>
    <p class="tune-hint">Ile panelu widać po prostowaniu (warp). Niżej = łatwiej o migawkę z daleka.</p>
    <div class="row">
  ...
  <div class="card" id="tunePanel">
    ...
    <div id="tuneHint" class="tune-hint">Zmiany bez restartu, widać po ok. 1 s.</div>
    <div class="tune-row">
  ...
  <div class="card" style="grid-column:1/-1">
    <h2>Parametry live</h2>
    <p class="param-desc">Wartości z bieżącej klatki. <strong>Reliable (moduł B)</strong> to główny sygnał jakości dopasowania siatki.</p>
...
function setRuntimeUi(rt) {
  ...
  const pct = Math.round(100 * Number(rt.snapshot_min_grid_overlap || 0.45));
  document.getElementById('snapOverlap').value = pct;
  document.getElementById('vSnapOverlap').textContent = pct;
  const pctU = Math.round(100 * Number(rt.snapshot_min_grid_overlap_unreliable || 0.45));
  document.getElementById('snapOverlapUnrel').value = pctU;
  document.getElementById('vSnapOverlapUnrel').textContent = pctU;
  const st = Number(rt.snapshot_min_stable || 1);
  document.getElementById('snapStable').value = st;
  document.getElementById('vSnapStable').textContent = st;
  const rp = Number(rt.snapshot_max_reproj || 25);
  document.getElementById('snapReproj').value = rp;
  document.getElementById('vSnapReproj').textContent = rp;
  const wp = Math.round(100 * Number(rt.snapshot_min_warp_coverage || 0.15));
  document.getElementById('snapWarp').value = wp;
  document.getElementById('vSnapWarp').textContent = wp;
}
function bindRuntimeSliders() {
  [['snapOverlap','vSnapOverlap',0],['snapOverlapUnrel','vSnapOverlapUnrel',0],
   ['snapStable','vSnapStable',0],['snapReproj','vSnapReproj',0],['snapWarp','vSnapWarp',0]].forEach(([id,vid]) => {
...
async function applyRuntime() {
  const body = {
    ...
    snapshot_min_grid_overlap_unreliable: Number(document.getElementById('snapOverlapUnrel').value) / 100,
    ...
    snapshot_min_warp_coverage: Number(document.getElementById('snapWarp').value) / 100,
  };
...
function renderStructuredParams(next) {
  ...
  let html = '';
  html += '<div class="param-block"><h3>Moduł A: podejście</h3>';
  html += '<dl class="param-row">';
  const wKadrze = present ? 'TAK' : (reason ? 'NIE (' + reason + ')' : 'NIE');
  html += '<dt>W kadrze</dt><dd class="' + valClass(present) + '">' + esc(wKadrze) + '</dd>';
  if (ma.ok) {
    ...
    html += '<dt>Reproj A</dt><dd>' + esc(ma.reproj_display || 'n/d') + '</dd>';
  } else if (Object.keys(ma).length) {
    html += '<dt>Status</dt><dd class="val-bad">FAIL</dd>';
    html += '<dt>Powód</dt><dd>' + esc(ma.reason || '?') + '</dd>';
  } else {
    html += '</dl><p class="param-desc">Wyłączony w tej sesji.</p>';
  }
  if (ma.ok || Object.keys(ma).length) html += '</dl>';
  html += '</div>';
  html += '<div class="param-block"><h3>Moduł B: skan</h3>';
  html += '<dl class="param-row">';
  ...
  if (mb.grid_line_match_ratio != null) html += '<dt>Linie</dt><dd>' + pct(mb.grid_line_match_ratio) + '</dd>';
  if (mb.warp_panel_coverage != null) html += '<dt>Warp</dt><dd>' + pct(mb.warp_panel_coverage) + '</dd>';
  html += '<dt>Rogi</dt><dd>' + esc(mb.corner_source) + '</dd>';
  html += '<dt>Backend</dt><dd>' + esc(mb.xy_backend) + '</dd>';
  html += '<dt>Panel</dt><dd>' + esc(mb.panel_id) + '</dd>';
  html += '</dl></div>';
  el.innerHTML = html;
}
...
async function fetchState() {
  ...
  const hint = m.mission_done
    ...
    ? `Panel ${m.current_panel || next.panel_id || 'A'} gotowy, raport na :8088 (klawisz ${{'A':'1','B':'2','C':'3'}[m.current_panel || next.panel_id || 'A'] || '?'} lub hold_started).`
    : m.panel_full === false && (next.snapshots || []).length
      ? 'Zbieranie migawek…'
      : 'Raporty z migawek (CXY). hold_started / 1·2·3 na :8088, 0 = wyczyść.';
...
def format_params_text(state: Dict[str, Any]) -> str:
    ...
    if ma or pp:
        ...
        if pp:
            lines.append(
                f"W kadrze: {'TAK' if pp.get('present') else 'NIE'}"
        ...
        if ma.get('ok'):
            ...
            lines.append(f"Reproj A: {ma.get('reproj_display', 'n/d')}")
        elif ma:
            lines.append(f"Status: FAIL")
            lines.append(str(ma.get('reason', '?')))
        elif not pp:
            lines.append('Wyłączony')
    ...
    if mb:
        ...
        lines.append(f"Panel: {mb.get('panel_id', '?')}")
        ...
        if mb.get('grid_line_match_ratio') is not None:
            lines.append(
                f"Linie: {100.0 * float(mb['grid_line_match_ratio']):.0f}%",
            )
        ...
        if mb.get('warp_panel_coverage') is not None:
            lines.append(
                f"Warp: {100.0 * float(mb['warp_panel_coverage']):.0f}%",
            )
    tr = p.get('tracker') or {}
    if tr:
        lines.append('')
    ...
    if rt:
        lines.append('')
        lines.append('PROGI MIGAWEK')
        lines.append(
            f"3 siatki: {100.0 * float(rt.get('snapshot_min_grid_overlap', 0)):.0f}%  "
    ...
    def _pick_stream_bgr(self) -> Optional[Any]:
        with self._stream_frame_lock:
            if self._stream_source == 'dashboard':
                # Pełny dashboard z parametrami; vis tylko zanim pierwszy dashboard jest gotowy.
    ...
    if module_a_po is not None:
        if getattr(module_a_po, 'ok', False):
            ma = {
                ...
                'reproj_display': pose_reproj_display_str(
                    float(module_a_po.reproj_mean_px), float(reproj_b),
                ),
            }
        else:
            ma = {
