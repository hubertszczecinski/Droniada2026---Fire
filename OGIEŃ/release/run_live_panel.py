"""Live: moduł A + B, dashboard WWW, migawki, integracja z autonomią."""
...
def _snapshot_competition_thresholds(
    ...
    *,
    ...
    if int(args.web_port) > 0 and min_ratio <= 0.0:
        min_ratio = 0.5
    # Przy luźnych progach migawek nie podbijaj do 3/5 — raport z 2 zgodnych migawek też OK.
    if int(args.snapshot_competition_min_votes) >= 3 and n_snapshots >= 5:
...
def _draw_vis_overlay(
    ...
    *,
    ...
    if draw_status and analysis_paused:
        from release.cv_text import put_text_utf8
...
def _stream_draw_status() -> bool:
    raw = os.environ.get('DRONIADA_STREAM_DRAW_STATUS', '0').strip().lower()
...
def _push_stream_vis_preview(
    ...
    vis = bgr.copy()
    if overlay_state:
        _draw_vis_overlay(vis, draw_status=_stream_draw_status(), **overlay_state)
...
def main() -> None:
    ...
    ap.add_argument('--snapshot-max-reproj', type=float, default=25.0,
    ...
    ap.add_argument('--snapshot-min-stable', type=int, default=1,
                    help='klatek z pokryciem siatki >= min-grid-overlap z rzędu przed zapisem')
    ap.add_argument('--snapshot-min-grid-overlap', type=float, default=0.45,
    ...
    ap.add_argument('--snapshot-max-reproj-a', type=float, default=28.0,
    ...
    ap.add_argument('--snapshot-min-grid-overlap-unreliable', type=float, default=0.45,
    ...
    ap.add_argument('--snapshot-min-warp-coverage', type=float, default=0.15,
                    help='min. pokrycie panelu na warpie (0=wył.; blokada tylko skrajnego tła)')
    ap.add_argument('--snapshot-require-yolo-corners', action=argparse.BooleanOptionalAction,
                    default=None,
                    help='migawka tylko przy rogach YOLO (nie cv_fallback na tle/trawie)')
    ap.add_argument('--snapshot-min-stand-confidence', type=float, default=0.0,
                    help='min. pewność stojaka modułu A (0=wył.; np. 0.45 na zawodach)')
    ...
    if args.bench:
        ...
        if float(args.snapshot_max_reproj) >= 25.0:
            args.snapshot_max_reproj = max(float(args.max_reproj_reliable), 25.0)
        if float(args.snapshot_max_reproj_a) >= 28.0:
            args.snapshot_max_reproj_a = max(float(args.max_reproj_reliable), 28.0)
    ...
    if args.snapshot_require_yolo_corners is None:
        _ry = os.environ.get('DRONIADA_SNAPSHOT_REQUIRE_YOLO_CORNERS', '0').strip().lower()
        args.snapshot_require_yolo_corners = _ry in ('1', 'true', 'yes', 'on')
    if args.snapshot_min_stand_confidence <= 0.0:
        _sc = os.environ.get('DRONIADA_SNAPSHOT_MIN_STAND_CONFIDENCE', '').strip()
        if _sc:
            try:
                args.snapshot_min_stand_confidence = float(_sc)
            except ValueError:
                pass
    ...
    if int(args.web_port) > 0:
        ...
        # Włączamy vis-preview dopiero po inicjalizacji kamery (GStreamer passthrough go wyłącza).
    ...
    def analysis_paused() -> bool:
        if mission_manager is not None and mission_manager.state.mission_done():
            return True
        # Pauza analizy tylko z autonomii WS (hold_stopped → autonomy_pause_until).
        # Faza depart lotu (po Wyślij / tier) nie wyłącza YOLO ani migawek.
        return time.monotonic() < autonomy_pause_until
    ...
    try:
        ...
        if args.video:
            ...
            if use_gst_cam:
                ...
                _gst_mjpeg_out = web_publisher is not None and (
                    _passthrough or int(_rotate or 0) != 0
                )
                src = GstMjpegCameraFeed(
                    ...
                    fps=_fps,
                    rotate=_rotate,
                    on_mjpeg=_mjpeg_cb if _gst_mjpeg_out else None,
                    on_bgr=_gst_bgr_cb,
                )
                if _gst_mjpeg_out:
            ...
            if use_gst_cam:
                ...
                for _ in range(80):
                    ...
                    ok_probe, bgr_probe, _ = src.get_latest()
                    if ok_probe and bgr_probe is not None:
                        _log(log_fh, f'[live_panel] kamera: {meta}')
                        if meta and meta.get('gstreamer_pipeline'):
                    ...
                    ok_probe, _, _ = src.get_latest()
                    if not ok_probe:
                        _gst_fallback = True
                        _log(log_fh, '[live_panel] GStreamer: brak klatek po 4s')

                if _gst_fallback:
                    _log(log_fh, '[live_panel] Fallback → OpenCV V4L2 (SharedCameraFeed)')
                    try:
                        src.stop()
                    except Exception:
                        pass
                    ...
                    if web_publisher is not None:
                        try:
                            web_publisher.enable_stream_passthrough(enabled=False)
                        except Exception:
                            pass
                    src = SharedCameraFeed(cam_cfg, on_frame=_cam_cb)

        use_camera_feed = not args.video
        _stream_src_mode = os.environ.get('DRONIADA_STREAM_SOURCE', 'vis').strip().lower()
        if vis_preview_worker is not None and (use_camera_feed or args.video):
            _passthrough = bool(
                web_publisher is not None and getattr(web_publisher, '_stream_passthrough', False)
            )
            _vis_on = _stream_vis_on and not _passthrough and _stream_src_mode != 'dashboard'
            vis_preview_worker.set_stream_enabled(_vis_on)
            _log(
                log_fh,
                f'[live_panel] vis-preview stream={"ON" if _vis_on else "OFF"} '
                f'(passthrough={_passthrough})',
            )
        with src:
            if use_camera_feed and not _gst_started:
                import time as _time
                for _ in range(50):
                    meta = getattr(src, 'open_meta', None)
                    if meta:
            ...
            video_realtime = bool(
                ...
                and (
                    show_preview
                    or args.out_video
                    or (args.headless and int(args.web_port) > 0)
                )
            )
            ...
            corner_meta: dict = {}
            panel_present = False
            panel_presence_reason = 'init'
            reliable = False
            reproj = 999.0

            def _video_pace() -> None:
                if args.video and video_realtime:
                    elapsed_ms = (time.monotonic() - loop_t0) * 1000.0
                    delay_ms = max(1, frame_period_ms - int(elapsed_ms))
            ...
            while True:
                ...
                _use_async = bool(_run_analysis and analysis_worker is not None)
                ...
                if not _run_analysis:
                    ...
                    h_mat = None
                    black_th = None
                # async: stan z poll_result — nie zeruj det/rogów/siatki między klatkami
                ...
                if args.dashboard:
                    ...
                    if snapshot_store is not None and not snapshots_paused() and (
                        ...
                        stand_conf = (
                            float(module_a_po.stand_confidence)
                            if module_a_po is not None and module_a_po.ok
                            else None
                        )
                        scene_ok, scene_reason = snapshot_scene_eligible(
                            panel_present=bool(panel_present),
                            corner_source=str(corner_src),
                            require_yolo_corners=bool(args.snapshot_require_yolo_corners),
                            module_a_stand_confidence=stand_conf,
                            min_stand_confidence=float(args.snapshot_min_stand_confidence),
                        )
                        if scene_ok:
                            ...
                            snap_ok, snap_reason = False, scene_reason
                        a_record: dict = {}
                        if module_a_po is not None and module_a_po.ok:
