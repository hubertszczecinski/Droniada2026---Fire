...
if [[ "${1:-}" == "droniada-vision" ]]; then
  ...
  exec python3 -m release.run_live_panel \
    ...
    --snapshot-max-reproj "${DRONIADA_SNAPSHOT_MAX_REPROJ:-20}" \
    --snapshot-min-grid-overlap "${DRONIADA_SNAPSHOT_MIN_GRID_OVERLAP:-0.45}" \
    --snapshot-min-grid-overlap-unreliable "${DRONIADA_SNAPSHOT_MIN_GRID_OVERLAP_UNRELIABLE:-0.68}" \
    --snapshot-min-stable "${DRONIADA_SNAPSHOT_MIN_STABLE:-1}" \
    --snapshot-max-reproj-a "${DRONIADA_SNAPSHOT_MAX_REPROJ_A:-22}" \
    --snapshot-min-warp-coverage "${DRONIADA_SNAPSHOT_MIN_WARP_COVERAGE:-0.25}" \
    --snapshot-min-stand-confidence "${DRONIADA_SNAPSHOT_MIN_STAND_CONFIDENCE:-0.45}" \
